    ...
    __attribute__((visibility("default"))) __attribute__((used)) char *cvProcess(int *inputImage, int *coorList, int *Labels, int *coorLen)
    {
        ...
        for (int i = 0; i < counter; i++)
        {
            ...
            if (0 <= x && 0 <= w && x + w <= 255 && 0 <= y && 0 <= h && y + h <= 255)
            {
                ...
                if (initThresh >= 0)
                {
                    // float mappedOuptput = sqrt((initThresh) / 0.00046) + 270;
                    float mappedOutput = 270;
                    // float mappedOutput = (0.005 * (initThresh * initThresh)) - (0.2 * initThresh) + 2;

                    arr1[1] = static_cast<int>(mappedOutput);
                    test_this = 30;
                }
                else
                {
        ...
        for (int quad = 1; quad <= 4; quad++)
        {
            ...
            if (flag)
            {
                ...
                if (initThresh >= 0)
                {
                    ...
                    float mappedOutput = 270;
                    // float mappedOutput = (0.005 * (initThresh * initThresh)) - (0.2 * initThresh) + 2;
                    noObjIntensity = static_cast<int>(mappedOutput);
                }
                else
                {
                    noObjIntensity = 0;
